...
def help_cmd(update, context):
    text = (
        ...
        "/debugcsv – show which CSV is used and columns\n"
        "/head – show first rows in chat\n"
...
def graph_cmd(update, context):
    try:
        ...
        if not img:
            ...
            return
        context.bot.send_photo(chat_id=update.effective_chat.id, photo=img, caption=f"📈 Equity Curve (source: {path})")
    except Exception as e:
        traceback.print_exc()
        update.effective_message.reply_text(f"❌ /graph failed: {e}")

# --------- Debug helpers ---------
def debugcsv_cmd(update, context):
    df, path = _load_df_safely()
    if df is None:
        update.effective_message.reply_text("No CSV loaded. Send a CSV file to the bot.")
        return
    cols = ", ".join(df.columns.tolist())
    update.effective_message.reply_text(f"CSV in use: `{path}`\nRows: {len(df)}\nColumns: {cols}", parse_mode=ParseMode.MARKDOWN)

def head_cmd(update, context):
    df, path = _load_df_safely()
    if df is None or df.empty:
        update.effective_message.reply_text("No data to show. Upload a CSV first.")
        return
    # send as preformatted text (first 5 rows)
...
def register_handlers(dispatcher):
    ...
    dispatcher.add_handler(CommandHandler("debugcsv", debugcsv_cmd))
    dispatcher.add_handler(CommandHandler("head", head_cmd))
